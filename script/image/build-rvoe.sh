...
# 前置依赖: sign-rv.sh
# 用法:
#   sudo ./script/image/build-rvoe.sh          # 离线模式 (默认)
#   sudo ./script/image/build-rvoe.sh --docker # Docker 在线模式 (跳过离线备份)
#
# rootfs 配置优先级:
...
FIX_RELOC="$WS/script/resources/fix_reloc.py"
...
for f in "$SHIM" "$MM" "$CSV" "$DB_KEY" "$DB_CRT" \
    ...
    [ -f "$f" ] || _die "缺少文件: $f  (请先运行 sign-rv.sh)"
done
