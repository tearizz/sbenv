#!/bin/bash
set -e
# sign-rv.sh — 签名 RISC-V EFI 组件
# 对 shim + 网络驱动执行 fix_reloc → SizeOfImage 扩展 → sbsign → 验证

_current_path=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)
PROJECT=$(dirname "$_current_path")
ARTIFACT="$PROJECT/artifact"
KEYS="$ARTIFACT/keys"
FIX_RELOC="$_current_path/resources/fix_reloc.py"
...
[ -f "$FIX_RELOC" ] || { echo "[ERROR] missing: $FIX_RELOC"; exit 1; }
...
sign_efi() {
    ...
    # fix .reloc Page RVA (RISC-V gnu-efi workaround)
...
# ---- shim + MokManager ----
for efi in "$ARTIFACT/shim/shimriscv64.efi" "$ARTIFACT/shim/mmriscv64.efi"; do
    [ -f "$efi" ] || { echo "[ERROR] missing: $efi (run shim build first)"; exit 1; }
    sign_efi "$efi"
done

# fallback (optional)
[ -f "$ARTIFACT/shim/fbriscv64.efi" ] && sign_efi "$ARTIFACT/shim/fbriscv64.efi"

# ---- network drivers (required for RISC-V) ----
for drv in Hash2DxeCrypto TcpDxe HttpDxe; do
    efi="$ARTIFACT/drivers/${drv}.efi"
    [ -f "$efi" ] || { echo "[ERROR] missing: $efi (extract from EDK2 build first)"; exit 1; }
    sign_efi "$efi"
done
